...
export function AddEnquireDialog({
  ...
  /**
   * Set form values when editing an enquiry
   */
  useEffect(() => {
    if (enquireToEdit) {
      // Only set _id when editing
      ...
      reset(); // Reset form when adding a new enquiry
    }
  ...
  const onSubmit = (data: IEnquire) => {
    ...
    onSubmitEnquire(payload as IEnquire); // Type assertion
    reset();
    onClose();
  };
  return (
